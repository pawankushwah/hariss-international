...
import { brandingList } from "@/app/services/allApi";
                    ...
                    config={{
                        header: {
                            ...
                            actions: [
                                <SidebarBtn
                                    key={0}
                                    href="/settings/manageAssets/branding/add"
                                    isActive
                        ...
                        api: {
                            list: async (pageNo: number, pageSize: number) => {
                                try {
                                    const res = await brandingList({ page: String(pageNo), limit: String(pageSize) });
                                    const formatted: Branding[] = (res.data || []).map(
                                        (s: Branding) => ({
                                            id: s.id,
                                            osa_code: s.osa_code,
                                            name: s.name,
