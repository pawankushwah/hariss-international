...
import { agentCustomerGenerateCode, addAgentCustomer } from "@/app/services/allApi";
...
const AddAgentCustomerPage = () => {
  ...
  const validateCurrentStep = async (step: number) => {
    ...
    if (step === 1) fields = [
      "agent_customer_code","tin_no", "name", "business_name", "customer_type",  "owner_name", "owner_no", "is_whatsapp", "whatsapp_no", "email", "language"
    ];
    if (step === 2) fields = [
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Prepare payload, convert string numbers to numbers where needed
      const payload = {
        ...form,
      ...
      const res = await addAgentCustomer(payload);
      if (res?.error) {
        showSnackbar(res.message || "Failed to add Agent Customer", "error");
      } else {
        showSnackbar("Agent Customer added successfully", "success");
        router.push("/dashboard/master/agentCustomer");
      }
    } catch (err) {
      showSnackbar("Add Agent Customer failed", "error");
    }
  };
    ...
    <>
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-semibold text-gray-900">Add Agent Customer</h1>
        </div>
      </div>
      <StepperForm
...
export default AddAgentCustomerPage;
