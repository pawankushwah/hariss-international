...
export default function Country() {
    ...
    const tableData: TableDataType[] = countries.map((c) => ({
        ...
        salesman_type_status: c.salesman_type_status ?? "",
    ...
    useEffect(() => {
        const fetchCountries = async () => {
            try {
                const listRes = await salesmanTypeList({});
                setCountries(listRes.data);
                console.log("Fetched salesman:", listRes.data);
            } catch (error: unknown) {
                console.error("API Error:", error);
            } finally {
                setLoading(false);
                        ...
                        rowActions: [
                            
                            {
                                ...
                                onClick: (data: object) => {
                                    ...
                                    router.push(`/dashboard/settings/salesman-type/update/${row.id}`);
                            ...
                            },
                            {
                                ...
                                onClick: (data: object) => {
                                    const row = data as TableRow;
                                    setSelectedRow({
                                        id: String(row.id),  // undefined भी आएगा तो "" (empty string) बन जाएगा
